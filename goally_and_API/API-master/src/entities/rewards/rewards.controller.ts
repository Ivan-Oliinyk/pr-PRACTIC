...
import { UpdateRewardOrdersDto } from './dto/UpdateRewardOrder.dto';
...
export class RewardsController {
  ...
  async updateOrderings(
    @Body() body: UpdateRewardOrdersDto,
    @Param('clientId', ParseObjectIdPipe) clientId,
    @UserFromReq() user: User,
  ) {
    const rewards = await this.rs.updateOrderings(clientId, body, user);
    return rewards;
  }
  ...
  async updateLibOrderings(
    ...
    const rewards = await this.rs.updateOrderings(null, body, user);
    return rewards;
  }
  ...
  @Get('/active-global')
  async getActiveGlobalRewards() {
    const routines = await this.rs.getActiveAdminRewards();
    return routines;
  }
}
